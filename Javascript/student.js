// class user, which is the main class where our sub classes gets a few of their attributes from
class User {
...
// Sub Class student which extends the user. Meaning: This class gets most of its attributes from the class user, but a few new ones, such as studyprogram and semester
...
// Log in for student - using Local storage.
// We start by creating an empty array called studentlist.
var studentList = [];
...
// When we have a class for our student, it will ask us for all of the data points that we defined in the class student and user.
// Here we push new student to our empty array:
...
// The studentlist is then made into a string using the JSON.stringify command
var studentListString = JSON.stringify(studentList);
// Here we set out studentListString in local storage so that we can later retrieve it.
localStorage.setItem('Student', studentListString);
    } else {
        studentList = JSON.parse(localStorage.getItem('Student'))
    }
/*
Not sure why this is here TBH - will figure out later
console.log(localStorage.getItem("Student"))
console.log(document.getElementById("category").value);
console.log(document.getElementById("subcategory").value);
*/
//Validation form that makes sure that you cannot join a lecture without selecting a study program, course and a lecture.
function studentValidationForm() {
// 4 variables that help keep track of the content within the dropdown lists and the comment box. Used in the "you have joined a lecture alert"
...
// Variable form_valid is set to true as a standard. When something is not selected the form_valid is set to false
    var form_valid = true;
// New variable for validation message. It is empty now since we dont know what the user has done wrong yet.
    var validation_message = "";
// This validation form works by first defining a new variable with the content of the dropdown list
// Here we simply say that if the content of the dropdown list is null or empty (e.g. not selected) the validation form (form_valid) is set to false and a new validation message is added.
// This is done for all 3 of the dropdown lists.
...
// "If form_valid" triggers if it is true. Form_valid default setting is true therefore it will happen if none of the 3 if statements above are triggered
// Alert that confirms which lecture you have joined
        if (form_valid) {
...
// If the form_valid is false it will trigger an alert with the validation message.
        else {
...
/*
// Function that shows the student information - not sure this have to be here. more of an experiment for the admin user.
